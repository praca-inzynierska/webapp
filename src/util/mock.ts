...
const mockStudents: Student[] = names.map((name, index) => new Student(index.toString(), `${name} ${surnames[index % surnames.length]}`, schools[index % schools.length]))
...
while (studentsCopy.length > 0) {
  mockTaskSessions.push(
    new TaskSessionModel(
      ...
      new Task(null, 'zadanie', 'opis', 'matematyka', 'whiteboard', 60, new Map()),
      123123))
}
